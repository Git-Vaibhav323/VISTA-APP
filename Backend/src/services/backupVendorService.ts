...
export const findBackupVendor = async (itemName: string, requiredQty: number, currentVendorId: string) => {
  ...
  // Filter out current vendor and sort by trustScore and (placeholder) distance
  ...
  return candidates.length > 0 ? candidates[0].vendor : null;
}; 